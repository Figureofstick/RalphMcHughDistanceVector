...
class Entity0(Entity):
    ...
    def update(self, pkt):
        ...
        # this checks to see if there are repeting changes
        if(self.diffInMinCost(previous, self.distance_table)):

            outgoing_packet = Packet(self.sim, 0, 1, self.getMyMinCost())
            self.sim.to_layer2(0, outgoing_packet)

            outgoing_packet = Packet(self.sim, 0, 2, self.getMyMinCost())
            self.sim.to_layer2(0, outgoing_packet)

            outgoing_packet = Packet(self.sim, 0, 3, self.getMyMinCost())
            self.sim.to_layer2(0, outgoing_packet)
